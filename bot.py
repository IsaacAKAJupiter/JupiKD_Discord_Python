...
bot = commands.Bot(command_prefix=get_prefix,
                   description="JupiKD, all purpose Discord bot. Created by: Isaac™#1240")
...
if __name__ in "__main__":
    for extension in initial_extensions:
        try:
            ...
            print(
                f"Failed to load extension {extension}. Error: {e}.", file=sys.stderr)
            traceback.print_exc()
...
@bot.event
async def on_ready():
    game = discord.Game(name="^help | isaacoram.ca")
    ...
    url = "http://localhost/old/API/jupikd_discord/updatebotuptime.php"
    params = {
...
async def on_command_completion(ctx):
    if ctx.command.cog_name != "OwnerCog" and ctx.command.cog_name != None:
        if await functions.AddUseToCommand(ctx) == False:
            print(
                f"ERROR: Adding use to Command | Command used: {ctx.command.name}")
        ...
        for i in ctx.command.checks:
            if (i.__qualname__) == "is_nsfw.<locals>.pred":
                embed = await functions.CreateEmbed(
                    author=(bot.user.display_name, discord.Embed.Empty,
                            bot.user.avatar_url_as(format="png")),
                    image="https://www.isaacoram.ca/images/nsfw.gif"
                )
                embed.add_field(
                    name="Error", value="You cannot use NSFW commands in a non-NSFW channel.")
