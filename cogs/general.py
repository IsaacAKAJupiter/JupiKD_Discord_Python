...
class GeneralCommands():
    ...
    async def twitch(self, ctx):
        ...
        embed = await functions.CreateEmbed(
            ...
            author=(self.bot.user.display_name, discord.Embed.Empty,
        ...
        embed.add_field(
            name="Link", value="[https://twitch.tv/jups](https://twitch.tv/jups)", inline=True)
    ...
    async def website(self, ctx):
        ...
        embed.add_field(
            name="Main", value="[https://www.isaacoram.ca](https://www.isaacoram.ca)", inline=True)
        embed.add_field(
            name="Bot Page", value="[https://www.isaacoram.ca/discordbot](https://www.isaacoram.ca/discordbot)", inline=True)
        embed.add_field(
            name="Bot Commands", value="[https://www.isaacoram.ca/discordbot/commands/](https://www.isaacoram.ca/discordbot/commands/)", inline=True)
        await ctx.send(embed=embed)
    ...
    async def github(self, ctx):
        ...
        embed.add_field(name="Repository Name",
                        value="JupiKD_Discord_Python", inline=True)
        embed.add_field(
            name="Link", value="[Github](https://github.com/IsaacAKAJupiter/JupiKD_Discord_Python)")
        await ctx.send(embed=embed)
    ...
    async def guilds(self, ctx):
        ...
        embed.add_field(name="Guild Count", value=len(
    ...
    async def commandcount(self, ctx):
        ...
        non_owner_commands = [
            command for command in self.bot.commands if command.cog_name != "OwnerCog" and command.cog_name != None]

        embed = await functions.CreateEmbed(
            author=(self.bot.user.display_name, discord.Embed.Empty,
                    "https://cdn-images-1.medium.com/max/230/1*OoXboCzk0gYvTNwNnV4S9A@2x.png")
        )
        embed.add_field(name="Command Count", value=len(
            non_owner_commands), inline=True)
        embed.add_field(
            name="Bot Commands", value="[Commands Link](https://www.isaacoram.ca/discordbot/commands/)", inline=True)
        await ctx.send(embed=embed)
    ...
    async def guild(self, ctx):
        ...
        embed.add_field(
            name="Name", value=ctx.message.author.guild.name, inline=True)
        embed.add_field(
            name="ID", value=ctx.message.author.guild.id, inline=True)
        embed.add_field(
            name="Owner", value=ctx.message.author.guild.owner.mention, inline=True)
        embed.add_field(
            name="Members", value=ctx.message.author.guild.member_count, inline=True)
        embed.add_field(name="Roles", value=len(
            ctx.message.author.guild.roles) - 1, inline=True)
        embed.add_field(name="Text Channels", value=len(
            ctx.message.author.guild.text_channels), inline=True)
        embed.add_field(name="Voice Channels", value=len(
            ctx.message.author.guild.voice_channels), inline=True)
        embed.add_field(name="Channels + Categories",
                        value=len(ctx.message.author.guild.channels), inline=True)
        embed.add_field(name="Emojis", value=len(
            ctx.message.author.guild.emojis), inline=True)
        await ctx.send(embed=embed)
    ...
    async def botinfo(self, ctx):
        ...
        url = "http://localhost/old/API/jupikd_discord/getuptime.php"
        params = {"key": config.jupsapikey}
        jsonURL = await functions.PostRequest(url, params)
        if jsonURL != None:
            start_time = datetime.datetime.strptime(
                jsonURL["uptime"], "%Y-%m-%d %H:%M:%S")
            uptime = (datetime.datetime.now()) - start_time
            uptime_string = await functions.TotalSecondsToDHMS(uptime.total_seconds())
        ...
        embed = await functions.CreateEmbed(
            ...
            footer=(
                f"Ping: {round(self.bot.latency * 1000)}ms | Uptime: {uptime_string}", discord.Embed.Empty)
        )
        embed.add_field(name="Creator", value="Isaac™#1240", inline=True)
        embed.add_field(
            name="Library", value=f"discord.py rewrite v{discord.__version__}", inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)
        embed.add_field(
            name="Invite", value="[Discord Link](https://discordapp.com/oauth2/authorize?client_id=362346139569094666&scope=bot&permissions=8&redirect_uri=https://www.www.isaacoram.ca/discordbot&response_type=code)", inline=True)
        embed.add_field(
            name="Website", value="[Jups Link](https://www.isaacoram.ca/discordbot)", inline=True)
        embed.add_field(
            name="Github", value="[Github Link](https://github.com/IsaacAKAJupiter/JupiKD_Discord_Python)", inline=True)
    ...
    async def invite(self, ctx):
        ...
        embed = await functions.CreateEmbed(
            author=(self.bot.user.display_name, discord.Embed.Empty,
                    self.bot.user.avatar_url_as(format="png")),
            image="https://www.isaacoram.ca/images/invite.gif"
        )
        embed.add_field(name="Click the link below to invite me to your guild!",
                        value="[Discord Invite Link](https://discordapp.com/oauth2/authorize?client_id=362346139569094666&scope=bot&permissions=8&redirect_uri=https://www.www.isaacoram.ca/discordbot&response_type=code)", inline=True)
        await ctx.send(embed=embed)
    ...
    async def top10(self, ctx):
        ...
        url = "http://localhost/old/API/jupikd_discord/top10usedcommands.php"
        params = {
        ...
        if not "success" in json:
            for i in json:
                if count % 2 == 0:
                    embed.add_field(name="\u200b", value="\u200b", inline=True)
                embed.add_field(
                    name=i["name"], value=f"Uses: {i['uses']}", inline=True)
                count += 1
    ...
    async def spotify(self, ctx, member=None):
        ...
        if actual_member.activity.type == discord.ActivityType.listening:
            ...
            embed.add_field(name="Song Name",
                            value=actual_member.activity.title)
    ...
    async def guildmembers(self, ctx):
        ...
        embed.add_field(name="People", value=len(
            [m for m in ctx.message.author.guild.members if m.bot is False]))
        embed.add_field(name="Bots", value=len(
            [m for m in ctx.message.author.guild.members if m.bot is True]))
        embed.add_field(name="Members Online", value=len(
            [m for m in ctx.message.author.guild.members if m.status is not discord.Status.offline]))
        embed.add_field(
            name="Total", value=ctx.message.author.guild.member_count)
        await ctx.send(embed=embed)
    ...
    async def steam(self, ctx, steamuser):
        ...
        if len(jsonURL["response"]["players"]) > 0:
            ...
            embed.add_field(
                name="Name", value=jsonURL["response"]["players"][0]["personaname"], inline=True)
            embed.add_field(name="Last Logoff", value=datetime.datetime.fromtimestamp(int(
                jsonURL["response"]["players"][0]["lastlogoff"])).strftime("%Y-%m-%d %H:%M:%S"), inline=True)
            embed.set_footer(
                text=f"Profile Link: {jsonURL['response']['players'][0]['profileurl']}")
            await ctx.send(embed=embed)
        else:
    ...
    async def commandinfo(self, ctx, command):
        ...
        embed.add_field(name="Description",
                        value=desc_permission[0], inline=False)
        embed.add_field(name="Default Permission",
                        value=desc_permission[1], inline=True)
        embed.add_field(name="Current Permission",
                        value=current_permission, inline=True)
